...
import fr.synchrone.glpisupport.presentation.items.viewmodel.helpers.ItemViewModelComputerHelper
...
@HiltAndroidTest
class ItemViewModelComputerHelperTests {
    ...
    private val helper = ItemViewModelComputerHelper(infos = ItemViewModelInfos(isCreation = false))

    private val itemToTest = BaseItemsContent.computerApi

    @Test
    fun test_if_getFieldsForDevice_works() {
        val result = helper.getFieldsForDevice(
            context = context,
            item = itemToTest
        )
    ...
    fun test_if_getUpdatedDeviceWithField() {
        ...
        val expectedItem = itemToTest.copy(
            ...
            stateId = newStateId,
            computerTypeId = newTypeId,
            computerModelId = newModelId,
    ...
    fun test_if_fetchModels_works() {
        runBlocking {
            ...
            assert(result.filterIsInstance(ComputerModelApi::class.java).size == result.size)
        }
    }
    ...
    fun test_if_fetchTypes_works() {
        runBlocking {
            ...
            assert(result.isNotEmpty())
            assert(result.filterIsInstance(ComputerTypeApi::class.java).size == result.size)
        }
    }

    @Test
    fun test_if_getEmptyItem_works() {
        assert(helper.getEmptyItem() == ItemViewModelEmptiesItems.emptyComputerApi)
    }
